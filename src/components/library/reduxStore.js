...
const reducer = (state = {
  ...
  action) => {
    ...
    if (typeof action.value === 'undefined') {
      if (!['counter/incremented', 'counter/decremented', 'clearSignedInUser']
    ...
    switch (action.type) {
      ...
      case 'addBullet':
        ...
        return { ...tempState,
          bullets: [...tempState.bullets, action.value],
        }
      ...
      case 'updateBulletByIndex':
        ...
        if (thisBullet
              && (!action.value.id || thisBullet.id === action.value.id)) {
          // make sure bullet exist and matches id if available
