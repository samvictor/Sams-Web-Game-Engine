// I took this from three/examples/jsm/loaders/GLTFLoader
// because I was getting a "Cannot use import statement outside a module" error
...
import { toTrianglesDrawMode } from 'three/examples/utils/BufferGeometryUtils.js';
...
class GLTFParser {
  ...
  loadImageSource(sourceIndex, loader) {
    ...
    if (sourceDef.bufferView !== undefined) {
      ...
      sourceURI = this.getDependency('bufferView', sourceDef.bufferView).then(function (bufferView) {
        isObjectURL = true;
  ...
  loadMaterial(materialIndex) {
    ...
    if (materialExtensions[EXTENSIONS.KHR_MATERIALS_UNLIT]) {
      ...
      if (metallicRoughness.baseColorTexture !== undefined) {
        pending.push(this.assignTexture(materialParams, 'map', metallicRoughness.baseColorTexture, SRGBColorSpace));
      }
    ...
    if (materialDef.emissiveTexture !== undefined && materialType !== MeshBasicMaterial) {
      pending.push(this.assignTexture(materialParams, 'emissiveMap', materialDef.emissiveTexture, SRGBColorSpace));
    }
  ...
  loadGeometries(primitives) {
    ...
    const createDracoPrimitive = (primitive) => {
      return extensions[EXTENSIONS.KHR_DRACO_MESH_COMPRESSION].decodePrimitive(primitive, this).then((geometry) => {
  ...
  loadAnimation(animationIndex) {
    ...
    return Promise.all([
      ...
      for (let i = 0, il = nodes.length; i < il; i++) {
        ...
        const createdTracks = this._createAnimationTracks(node, inputAccessor, outputAccessor, sampler, target);
  ...
  _loadNodeShallow(nodeIndex) {
    ...
    const meshPromise = this._invokeOne(function (ext) {
    ...
    if (nodeDef.camera !== undefined) {
      pending.push(
        this.getDependency('camera', nodeDef.camera).then((camera) => {
          return this._getNodeRef(this.cameraCache, nodeDef.camera, camera);
        }),
      );
    }
